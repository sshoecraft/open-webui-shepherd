...
export const uploadBrandingLogo = async (token: string, file: File) => {
	let error = null;

	const formData = new FormData();
	formData.append('file', file);

	const res = await fetch(`${WEBUI_API_BASE_URL}/configs/branding/logo`, {
		method: 'POST',
		headers: {
			Authorization: `Bearer ${token}`
		},
...
export const deleteBrandingLogo = async (token: string) => {
	let error = null;
